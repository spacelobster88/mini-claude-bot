...
def convert_markdown_tables(text: str) -> str:
    ...
    while i < len(lines):
        ...
        if not in_code_block and lines[i].strip().startswith('|'):
            ...
            # Build aligned text lines with pipe separators and header underline
            aligned: list[str] = []
            for ri, row in enumerate(rows):
                ...
                if ri == 0:
                    underline_parts = ['─' * w for w in col_widths]
                    aligned.append('─┼─'.join(underline_parts))
...
class SessionManager:
    ...
    _HARNESS_PROMPT_NORMAL = (
        ...
        "  Then run a grill-me-style interview — for EACH clarifying question also propose your own recommended answer "
        "(format 'question -> my suggestion -> why') so the user can just confirm or correct one word; "
        "relentlessly sharpen the plan/design: surface ambiguities, edge cases and risks. "
