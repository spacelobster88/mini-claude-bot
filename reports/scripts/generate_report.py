...
# Contacts
CONTACTS = {
    "eddie": "xinhant@gmail.com",
    "dad": "575805316@qq.com",
    "erin": "qifangerin@gmail.com",
}
SENDER = "spacelobster88@gmail.com"
...
FINAL LINE: Output a short sweet love note for Erin (1-2 sentences). Format:
LOVENOTE: Every morning is brighter knowing I get to share this world with you, Erin."""

LOVE_NOTE_DEFAULT = "Wishing you a beautiful day filled with joy, Erin."
...
def generate_chinese_report(preview: bool = False):
    ...
    if preview:
        send_email(to=CONTACTS["eddie"], cc="", bcc="", subject=f"[PREVIEW] {subject}", body=body, attachment=str(pdf_path.resolve()))
    else:
        send_email(to=CONTACTS["dad"], cc=CONTACTS["eddie"], bcc=CONTACTS["erin"], subject=subject, body=body, attachment=str(pdf_path.resolve()))
...
def generate_english_report(preview: bool = False):
    ...
    body = f"Good morning Erin!\n\nYour daily report is attached.\n\n{love_note}\n\n---\nAuto-generated by mini-claude-bot"

    if preview:
        send_email(to=CONTACTS["eddie"], cc="", bcc="", subject=f"[PREVIEW] {subject}", body=body, attachment=str(pdf_path.resolve()))
    else:
        send_email(to=CONTACTS["erin"], cc=CONTACTS["eddie"], bcc="", subject=subject, body=body, attachment=str(pdf_path.resolve()))
...
def main():
    ...
    parser.add_argument("--preview", action="store_true", help="Send to Eddie only for preview")
